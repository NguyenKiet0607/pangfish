...
@section('content_header')
    <h1>
        <a href="{{ route('admins.index') }}"><i class="fa fa-fw fa-user-secret"></i>
            {{ __('layouts.admins.title') }}</a> / {{ __('layouts.create') }}
    </h1>
@stop
...
@section('content')
    ...
    <div class="card card-outline card-success">
        ...
        <div class="card-body">
            <form class="form-horizontal" action="{{ route('admins.store') }}" method="post" id="admin-form-create" enctype="multipart/form-data" autocomplete="off" data-env="{{ env('APP_URL') }}">
                {{ csrf_field() }}
